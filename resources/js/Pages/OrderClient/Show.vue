<template>
    <div class="min-h-screen bg-gray-50 py-8">
        <div class="container mx-auto px-4 max-w-4xl">
          ...
          <!-- Dynamic Flash Message (Fixed at top) -->
          <transition name="fade">
            <div v-if="flashMessage"
                 :class="[
                         'fixed top-4 left-1/2 transform -translate-x-1/2 z-50 px-6 py-4 rounded-lg flex items-center gap-3 shadow-lg transition-all duration-300',
                         flashType === 'success'
                             ? 'bg-green-50 border border-green-200 text-green-800'
                             : 'bg-red-50 border border-red-200 text-red-800'
                     ]">
              <svg v-if="flashType === 'success'" class="w-5 h-5 text-green-600" fill="currentColor"
                   viewBox="0 0 20 20">
                <path clip-rule="evenodd"
                      d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                      fill-rule="evenodd"></path>
        ...
        call(phoneText, encryptedPhone) {
            ...
            axios.post(route('employee.call'), {
                ...
                .then(response => {
                  ...
                  this.showFlashMessage('Звонок запущен', 'success');
                })
                .catch(error => {
                    console.error('Error:', error);
                  // Show error message
                  this.showFlashMessage('Ошибка при запуске звонка', 'error');
                });
        },
